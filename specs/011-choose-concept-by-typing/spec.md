...
A developer adding this package to a project installs it, adds one entry to the project's URL configuration, and every form in the project that carries one of these fields now has search-as-you-type. They declare no widget, write no view, and add nothing to any form class.
...
4. **Given** the shipped documentation, **When** the README is read, **Then** the route include is shown, along with what it exposes and how a project restricts it.
...
- **FR-002**: The package MUST carry the search endpoint in its own URL configuration, for a project to include once at an address of the project's choosing. The control MUST resolve that address rather than assume one, so a project's choice of prefix is honoured.
...
- **FR-010**: The system check the package already contributes MUST report a project that has not included the package's route, identifying what to add. It MUST remain a warning rather than an error where the existing check is, and MUST NOT query the database while running.
