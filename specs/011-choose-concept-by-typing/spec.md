...
4. **Given** the shipped documentation, **When** the README is read, **Then** both wiring steps are shown in the order a developer does them, along with what the endpoint exposes and how a project restricts it.
...
- **FR-002**: The package MUST carry the search endpoint in its own URL configuration, for a project to include once at an address of the project's choosing. The control MUST resolve that address rather than assume one, so a project's choice of prefix is honoured. *(Amended at the plan stage, `decisions.md` D10: a project also declares the control's supporting package among its installed applications, because that is how Django finds another package's templates and static files. Two steps, both reported by the system check when missing.)*
...
- **FR-010**: The system check the package already contributes MUST report a project that has not included the package's route, and a project that has not declared the control's supporting package among its installed applications, identifying in each case what to add. It MUST remain a warning rather than an error where the existing check is, and MUST NOT query the database while running.
