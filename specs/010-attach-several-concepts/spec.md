...
### User Story 2 - Every selection stays inside the named vocabulary (Priority: P1)

A concept from some other vocabulary cannot end up in a record's set. A developer who attaches one is refused, and a form built from the model offers only concepts from the named vocabulary in the first place.

**Why this priority**: The constraint is the whole difference between this field and a plain many-to-many to `Concept`. A guarantee that holds when a form is submitted but not when the relation is written directly is not a guarantee, because writing the relation directly is how a many-valued field is ordinarily used.

**Independent Test**: With two vocabularies in the database, attach a concept from the wrong one directly through the relation and confirm the refusal, submit a form carrying one and confirm the rejection, then inspect a generated form's choices and confirm only the named vocabulary's concepts appear.

**Acceptance Scenarios**:

1. **Given** two vocabularies each holding concepts, **When** a concept from the vocabulary the field does not name is attached through the relation, **Then** it is refused with a message naming the expected vocabulary, and the record's set is unchanged.
2. **Given** the same record, **When** several concepts are attached at once and one of them is from another vocabulary, **Then** the whole write is refused and the record's set is unchanged.
3. **Given** a form generated from the consuming model, **When** its choices are inspected, **Then** every one is a concept of the named vocabulary and no concept of any other vocabulary appears.
4. **Given** a form submission carrying a concept from another vocabulary, **When** the form is validated, **Then** it is rejected rather than saved.
5. **Given** concepts from the correct vocabulary, **When** they are attached, **Then** the write succeeds.
...
- A form is rendered against a vocabulary with tens of thousands of concepts. It works and it is slow, and making it fast is [#88](https://github.com/SamuelJennings/django-controlled-vocabularies/issues/88).
...
- **FR-002**: The declaration MUST name exactly one vocabulary, by that vocabulary's slug, and MUST resolve it without requiring the vocabulary to exist when the declaration is read. Naming no vocabulary MUST be refused, because an unconstrained field is a plain many-to-many and offers none of this field's guarantees.
...
- **FR-004**: The system check the package already contributes MUST cover this field too, reporting a declared field whose named vocabulary is absent under the same identifier and the same warning-not-error rule as the single-value field.
- **FR-005**: Attaching a concept that does not belong to the named vocabulary MUST be refused, with a curator-facing message naming the expected vocabulary. The refusal MUST hold when the relation is written directly, not only when a form is validated, and a write carrying several concepts of which any is invalid MUST be refused whole, leaving the record's set unchanged.
...
- **FR-013**: `CONTEXT.md` MUST define the term this feature puts into a consuming project's code. The README MUST document declaring the field, reading labels and identifiers back, and what required and optional mean for it. The CHANGELOG MUST record the addition.
...
- **SC-002**: No write path that passes through the relation or a form puts a concept from outside the named vocabulary into a record's set, and no form offers one.
