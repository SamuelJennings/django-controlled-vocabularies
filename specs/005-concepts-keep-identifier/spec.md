...
- **FR-004**: An externally assigned static URI MUST be a well-formed absolute identifier carrying a scheme, MUST use a scheme on the configured allowlist (`http`, `https`, `urn`, `doi`, `info`, `ark`, `tag`, `hdl`, and `oai` by default — decisions.md D15), MUST NOT contain a control character, and MUST NOT exceed 500 characters. A value failing any of these MUST be refused with a translatable message rather than stored. A value that could never be resolved would corrupt the identity guarantee this feature exists to provide, an unlisted or control-bearing value becomes a hazard the moment the browsing interface renders the identifier as a link (a script-bearing scheme such as `javascript:` is refused twice over: it is not on the allowlist, and it is refused explicitly even if a downstream project's own override adds it), and a value beyond the length bound cannot be covered by the unique index FR-006 requires on every supported database.
