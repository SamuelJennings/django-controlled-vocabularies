...
- **Q: Which identifiers may an external publisher assign — any absolute URI, or a restricted set?** → A: A restricted set: a scheme on a small, configured allowlist — `http`, `https`, `urn`, `doi`, `info`, `ark`, `tag`, `hdl`, and `oai` by default. A stored identifier is rendered as a link by the browsing interface later, so the accepted set needs to be small and known-safe rather than merely known-not-hostile. An earlier draft of this rule refused only schemes that can carry executable content (`javascript:`, `data:`, `vbscript:`) and accepted everything else, which also let through `file:`, `about:`, and other schemes with no legitimate use here; that denylist shape was superseded by this allowlist (decisions.md D15), and the three script-bearing schemes are still refused as a second, belt-and-braces gate even if a downstream project's own allowlist override includes one. A strict allowlist of just `http` and `https` was rejected first, because real vocabularies legitimately use `urn:` identifiers, and refusing them would discard valid content. Escaping at render time remains R6's responsibility as well. Integrated into FR-004.
...
- **FR-002**: An externally assigned static URI MUST be held verbatim: never composed, never recomputed from the site's configured address or the record's slugs, and never normalised or re-cased. The application MUST NOT itself overwrite a stored value. It MUST NOT, however, attempt to defend a stored value against deliberate modification through every write path — keeping a published identifier still is a data and interface concern, met by making the field non-editable wherever it is exposed once a record is published, and a value that differs between two imports of the same vocabulary is a defect in the source file (decisions.md D18, superseding the earlier "never changed once stored" framing). A re-import matches an existing record *by* its identifier and therefore has no occasion to rewrite it.
...
- **FR-004**: An externally assigned static URI MUST be a well-formed absolute identifier carrying a scheme, MUST use a scheme on the configured allowlist (`http`, `https`, `urn`, `doi`, `info`, `ark`, `tag`, `hdl`, and `oai` by default — decisions.md D15), and MUST NOT exceed 500 characters. A value failing any of these MUST be refused with a translatable message rather than stored. A value that could never be resolved would corrupt the identity guarantee this feature exists to provide, an unlisted value becomes a hazard the moment the browsing interface renders the identifier as a link (a script-bearing scheme such as `javascript:` is refused twice over: it is not on the allowlist, and it is refused explicitly even if a downstream project's own override adds it), and a value beyond the length bound cannot be covered by the unique index FR-006 requires on every supported database.
...
- **FR-006**: No two records **of the same model** MAY hold the same static URI, enforced by a database constraint rather than by application code. Uniqueness across the three models is NOT a requirement (decisions.md D18): a record's place in its vocabulary is already unique by construction — R1 gives `ConceptScheme.slug` a site-wide unique constraint and `Concept` and `Collection` a unique `(scheme, slug)` each — so a composed URI cannot collide, and for two *stored* identifiers to collide across models a source file would have to declare the same URI for a `skos:Concept` and a `skos:Collection`, which asserts they are the same resource and is a malformed file. #50's importer reports such a file while reading it, which is a better error than a constraint violation.
...
- **FR-011**: Indexing MUST be deliberate: the static URI is the column identity lookups run against and MUST be indexed, its per-model uniqueness (FR-006) MUST be a database constraint, and any queryable-but-unindexed field this feature adds MUST be a recorded decision.
...
- **FR-013**: Fixedness moves in one direction only. A record's static URI becomes fixed when the record is created from external content, and no ordinary edit, rename, or re-import turns a fixed identifier back into a provisional one — nothing in the application ever clears a stored value. (The second and only other way a record becomes fixed is publication, which R4 owns.) As with FR-002, this is a property of what the application does, not a guarantee enforced against a caller who sets the field to nothing deliberately (decisions.md D18).
