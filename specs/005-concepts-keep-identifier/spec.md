...
- **"Static" is earned at publication.** A locally authored, unpublished record's URI is never absent — it is dynamic, computed from the configured base address and the record's slugs, and follows a rename until the value is fixed. `static_uri` stays unset until an external publisher assigns one, or R4 freezes it at publication; the value it carries in the meantime may still change. One field holds both states because the difference is fixedness, not presence. Confirmed at grilling.
