...
- **Q: The hierarchy is stored as edges, and the restriction is a transitive read over them. What if the stored edges contain a cycle?** → A: The traversal terminates and yields each concept once. The relation model refuses a self-relation and a mirror-order duplicate of a symmetric relation, but a reversed hierarchy edge is a different and permitted edge, and nothing in it walks the graph, so a cycle is not currently prevented from being stored. A restriction that hangs or exhausts memory on data the database was willing to accept is a defect regardless of how the data got there. Integrated into FR-004 and Edge Cases.
