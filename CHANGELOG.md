...
- Every concept and every collection now has its own address and its own page: a definition list
  ...
  holds none. Every record-valued row is a short in-site form linking to that record's own page,
  carrying its full canonical identifier as ordinary text beside it rather than only in an
  attribute a pointer reveals. Both pages show what kind of record it is and its own identifier as
  a link, and both are **read-only**, with no permission rule of their own. An address naming no
  record, or naming no vocabulary at all, is reported not found the same way. The number of
  queries either page runs does not grow with how much it shows. See the README.
...
- A concept's own page now also names, below its definition list, every collection that gathers
  ...
  section carries a plain-language heading rather than a SKOS property. A concept no collection
  gathers shows no such section.
