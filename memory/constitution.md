...
1. **The Python/Django API** covers `ConceptField`/`ConceptsField`, models, and the import/export
   surface, governed by semantic versioning with a one-minor-version deprecation window after 1.0.
2. **The vocabulary data contract** covers the concept **URIs** and the **RDF serialization** the app
...
- **Managed vocabularies round-trip losslessly:** for vocabularies authored and managed here, the
  unknown predicate tail is preserved verbatim as escrow and re-emitted on export, so nothing the
...
- Export emits correct RDF types (`URIRef` vs `Literal` vs typed literal) via the predicate
  registry, so types are declared, never guessed from string shape.
