...
- **Move identity into one shared table with a row per record.** It would let a single unique index span all three models. It costs a join on every read of a record's identity, to prevent a collision that needs a source file to hand two different kinds of record the same URI — which in RDF asserts they are the same resource, so the file is malformed and #50's importer reports it while reading. Rejected, and now moot: uniqueness across models is no longer a requirement at all (decisions.md D18). Uniqueness within a model is a database constraint, and identity is unique by construction anyway, because a vocabulary's slug is unique site-wide and a concept's or collection's slug is unique within its vocabulary.
