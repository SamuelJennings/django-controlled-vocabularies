...
- The URI turns **static** when the record arrives from an outside publisher, and later when the record's vocabulary is published. A static URI is stored in the `static_uri` column, is returned verbatim, and can never be rewritten or cleared.
