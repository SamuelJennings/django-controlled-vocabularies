...
The import command's `--rehearse` is the first instance.
...
A large share of the outcomes a curator runs a rehearsal to discover are refusals raised by the
...
which is the precise failure a rehearsal exists to prevent.
...
previews. Running the real thing and discarding it means the report a rehearsal produces is
produced by the same code that produces a live one, so "the rehearsal matches the live run" is true
by construction rather than by two implementations agreeing on the day they were written.

It also makes the guarantee testable as an equality: a rehearsal and a live run against the same
...
The cost is real and was accepted: a rehearsal takes as long as the import and does the same
database work before throwing it away. For a vocabulary of any realistic size that is seconds, and
an operator who asked for a rehearsal has already accepted waiting.

One consequence has to be handled deliberately: because the two renderings are identical by
construction, a rehearsal's output must say so in its own text, or a reader scrolling back cannot
