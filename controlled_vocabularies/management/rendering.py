...
(T015, T016, T017, FR-006, FR-007, FR-008, plan.md "Rendering").
...
per-language account) and records absent from the source, named in their own section, visibly
separate from set-asides and never counted among them (decisions.md D7). Every section prints,
whatever it holds: a section reading zero and a section silently missing look identical to a
reader but mean different things (FR-008's own reasoning for ``absent_from_source``, applied here
to every bucket) — which is also why each grouping below is a plain iteration over the report's
own accessors: an empty grouping simply yields nothing, and the bucket count above it already
shows the zero. Per-entry detail at raised verbosity is US-4's own later task, added to this
class next rather than anticipated here.
"""
...
class ReportRenderer:
    ...
    def render(self) -> Iterator[str]:
        ...
        then the per-language account), then the records absent from the source, then the
        rehearsal line."""
        yield str(
