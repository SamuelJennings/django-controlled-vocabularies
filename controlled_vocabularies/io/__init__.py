...
Nothing here is exported yet beyond this docstring — the reader itself
(``skos.py``) and its public ``import_skos()`` entry point are a later story
(tasks.md Phase US-1). This module grows its re-exports one task at a time,
each landing with the test that covers it.
"""
