...
| **Rehearsal** | A `--rehearse` run of the `import_skos` management command: performs the entire import, in a transaction that is always rolled back, and reports exactly what a live run would. | Not a lightweight validation that never touches the database — it does the same work a live run does and takes as long, so its report reflects every outcome a live run would, including ones only a database write itself would refuse. |
