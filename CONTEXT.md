...
| **Static URI** (`static_uri`) | The identifier once it is fixed: assigned by an external publisher and held exactly as given, or frozen when this site publishes the vocabulary (R4). Empty (`None`) means the record's `uri` is still dynamic. | Was called "Permanent URI". Fixed once externally assigned or once published; never rewritten before then, either. Not the same as **URI** — it is one of the two states `uri` can be in. |
