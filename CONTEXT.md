...
| **Concept search control** | The search-as-you-type widget `ConceptField` and `ConceptsField` render as by default in any Django form built from a consuming model — no widget declared, no endpoint written. Backed by this package's own search endpoint (`controlled_vocabularies.urls`), restricted at search and validation time to whatever vocabularies the field declares, carrying no permission rule of its own. | Delivered by #88. Needs a browser running JavaScript; without one, the field still works as an ordinary required-relation form field. See the README's "Choosing a concept by typing". |
