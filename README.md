...
import/export boundary. That puts everything Django already does well (forms, permissions, indexed
search, referential integrity) to work on vocabulary management, and lets vocabularies scale to tens
of thousands of concepts and evolve as data rather than code.
...
- A reasoner; there is no OWL inference.
...
- A faithful mirror of imported external vocabularies. Imports are normalised to what the app
  supports (e.g. its configured languages), and unsupported languages and constructs are not stored.
...
Supersedes and retires `skos-builder` and `django-research-vocabs`, bringing vocabulary authoring,
management, and Django consumption into one app.
